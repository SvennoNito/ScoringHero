...
class SpectogramWidget(QtWidgets.QWidget):
    ...
    def draw_spectogram(self, power, freqs, freqsOI, config):
        ...
        index_of_desired_tick_values_in_freqs   = np.unique([(np.abs(freqs - tick_value)).argmin() for tick_value in desired_tick_values])
        desired_tick_string                     = list(map(str, [int(y) for y in index_of_desired_tick_values_in_freqs * freqres]))
        desired_tick_tuple                      = [(val, text) for val, text in zip(index_of_desired_tick_values_in_freqs, desired_tick_string)]
