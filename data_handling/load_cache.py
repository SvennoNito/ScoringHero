...
def load_cache(ui):
    ...
    if os.path.exists(filename):
        ...
        is_same_spectogram_parameters = (
            ...
            and cache["Channel_index_for_spectogram"]
            == ui.config[0]["Channel_index_for_spectogram"]
        )

    # Spectogram
    if "spectogram_power" not in cache or not is_same_spectogram_parameters:
        (
            cache["spectogram_power"],
            cache["spectogram_freqs"],
            cache["spectogram_freqsOI"],
            cache["swa"],
...
def cache_to_ui(ui, cache):
    ui.power, ui.freqs, ui.freqsOI, ui.swa = (
        cache["spectogram_power"],
        cache["spectogram_freqs"],
        cache["spectogram_freqsOI"],
        cache["swa"],
