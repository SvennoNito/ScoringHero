...
def turn_into_epochs(times_vector, epolen, srate, extend_l, extend_r):
    ...
    end_indices_ext[end_indices_ext > int(np.floor(times_vector[-1] * srate))] = int(
        np.floor(times_vector[-1] * srate)
    )

    for start_ext, end_ext, start, end in zip(
        start_indices_ext, end_indices_ext, start_indices, end_indices
    ):
