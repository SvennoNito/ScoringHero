...
@timing_decorator
def times_extended_epoch(
    times_and_indices, this_epoch, epolen, extension_l, extension_r
):
    ...
    [
        ...
        for times, indices in times_and_indices[
            this_epoch - extend_by_numepo_l : this_epoch
        ]
    ]
    ...
    extended_times = np.concatenate(
        (times_l[extend_by_l:], times, times_r[:extend_by_r])
    )
...
def find_closest_index(number_list, target):
    if len(number_list) > 0:
        closest_index = min(
            range(len(number_list)), key=lambda i: abs(number_list[i] - target)
        )
    else:
        closest_index = None
