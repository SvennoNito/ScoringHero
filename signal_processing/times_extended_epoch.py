...
def times_extended_epoch(times_and_indices, this_epoch, epolen, extension_l, extension_r):
    ...
    times_r = []; indices_r = []; [(times_r.extend(times), indices_r.extend(indices)) for times, indices in times_and_indices[this_epoch+1 : this_epoch+extend_by_numepo_r+1]]
    times_l = []; indices_l = []; [(times_l.extend(times), indices_l.extend(indices)) for times, indices in times_and_indices[this_epoch-extend_by_numepo_l : this_epoch]]
    ...
    extended_indices    = np.concatenate((indices_l[extend_by_l:], indices, indices_r[:extend_by_r])).astype(int)
